...
export class SauceService {
  ...
  public sauces$ = new Subject<Sauce[]>();
  ...
  likeSauce(id: string, like: boolean) {
    return new Promise((resolve, reject) => {
    this.http.post("http://localhost:3000/api/sauces/" + id + "/like" , { userId: this.auth.userId, like: like ? 1 : 0 }).subscribe(
             (response) => {
                resolve(like);
         //       console.log(like);
            },
...
dislikeSauce(id: string, dislike: boolean) {
    return new Promise( (resolve, reject) => {
        this.http.post("http://localhost:3000/api/sauces/" + id + "/like" , { userId: this.auth.userId, like: dislike ? -1 : 0 }).subscribe(
            (response) => {
                resolve(dislike);
              //  console.log(dislike);
            },
  ...
  createNewThing(sauce: Sauce) {
    return new Promise((resolve, reject) => {
      this.http.post('http://localhost:3000/api/sauces', sauce).subscribe(
        (response) => {
          resolve(response);
        },
        (error) => {
  ...
  createNewThingWithFile(sauce: Sauce, image: File) {
    return new Promise((resolve, reject) => {
      const sauceData = new FormData();
      sauceData.append('sauce', JSON.stringify(sauce));
      sauceData.append('image', image, sauce.name);
      this.http.post('http://localhost:3000/api/sauces', sauceData).subscribe(
        (response) => {
          resolve(response);
        },
        (error) => {
  ...
  modifyThing(id: string, sauce: Sauce) {
    return new Promise((resolve, reject) => {
  ...
  modifyThingWithFile(id: string, sauce: Sauce, image: File | string) {
    return new Promise((resolve, reject) => {
      let sauceData: Sauce | FormData;
      if (typeof image === 'string') {
        sauce.imageUrl = image;
        sauceData = sauce;
      } else {
        sauceData = new FormData();
        sauceData.append('sauce', JSON.stringify(sauce));
        sauceData.append('image', image, sauce.name);
      }
      this.http.put('http://localhost:3000/api/sauces/' + id, sauceData).subscribe(
        (response) => {
          resolve(response);
        },
        (error) => {
